...
def assignment_penalty(data):
    ...
    cost_matrice = [students[i][1] for i in range(n_students)] #Matrice des pénalités
    ...
    #Matrice des allocations (binaire) pour l'étudiant i, si le sujet j lui est attribué, la valeur est de 1, 0 sinon
    ...
    model = pl.LpProblem("Affectation-penalty-problem",pl.LpMinimize)

    #Fonction objective, on cherche à minimiser la pénalité globale
    obj_func = pl.lpSum(cost_matrice*isAllocated)
    model += obj_func
    ...
    #Gestion du cas où la résolution est impossible
    if flag != 1:
        return ({"error" : "Solving is not possible with input data, check constraints and input values."},flag)

    #Récupération des résultats
    results = generate_results(topic_per_student,students_list,topics_list,model) #Dictionnaire stockant les résultats
    #Récupération des stats
    stats = generate_stats(topic_per_student,results,cost_matrice,students_list,topics_list,model,"penalty")
